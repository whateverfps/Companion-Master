...
drawingRenderedEventTarget.addEventListener(DrawingRenderedEvent, event => {
  ...
  try {
    ...
    if (detail.pageId && detail.projectId) {
      ...
      // Log once for the active page to diagnose mapping vs display
      const newLinks = drawingSpecificationLinks.forPage(detail.pageId);
      const confirmedCount = newLinks.filter(link => link.status === 'confirmed').length;
      const suggestedCount = newLinks.filter(link => link.status === 'suggested').length;
      const sectionNumbers = newLinks.map(link => link.sectionNumber);
      console.log('[Specification Panel Diagnostics]', {
        pageId: detail.pageId,
        projectId: detail.projectId,
        confirmedCount,
        suggestedCount,
        sectionNumbers,
        origins: newLinks.map(link => link.origin)
      });
    }
...
function constructionIntelligencePanelMarkup(model) {
  ...
  const specificationCards = items => `<ol class="mc-ci-specifications">${items.map(item => `<li><div><strong>${esc(item.sectionNumber)}</strong><span>${esc(item.sectionTitle)}</span></div><div class="mc-ci-spec-meta"><span class="mc-ci-badge ${esc(item.displayStatus)}">${esc(item.displayStatus)}</span><span>${Math.round((Number(item.confidence) || 0) * 100)}% confidence</span><span class="mc-ci-source-label">${esc(item.evidenceSource || item.origin || 'drawing-evidence')}</span>${item.applicabilityScopes?.length ? `<span>${esc(item.applicabilityScopes.join(' · '))}</span>` : ''}</div>${item.reason ? `<p class="mc-ci-spec-reason">${esc(item.reason)}</p>` : ''}${item.evidenceText ? `<div class="mc-ci-evidence"><strong>${esc(item.evidenceSource || 'Drawing evidence')}</strong><p>${esc(item.evidenceText)}</p></div>` : ''}${item.evidence?.length ? `<details class="mc-ci-evidence"><summary>Supporting evidence (${fmt(item.evidence.length)})</summary><ul>${item.evidence.map(evidence => `<li><strong>${esc(evidence.evidenceType || evidence.source || 'Drawing evidence')}</strong><span>${esc(evidence.evidenceText || evidence.text || '')}</span></li>`).join('')}</ul></details>` : ''}<div class="mc-ci-actions">${item.canShowSource ? `<button data-object-spec-source="${esc(item.specificationDocumentId)}" data-object-spec-page="${item.sourcePageNumber}" data-object-spec-section="${esc(item.sectionNumber)}">View Source</button>` : ''}${item.status === 'suggested' && item.relationshipId ? `<button data-project-relationship-confirm="${esc(item.relationshipId)}">Confirm</button><button data-project-relationship-reject="${esc(item.relationshipId)}">Reject</button>` : item.status === 'suggested' && item.drawingSpecLinkId ? `<button data-drawing-confirm-spec="${esc(item.drawingSpecLinkId)}">Confirm</button><button data-drawing-reject-spec="${esc(item.drawingSpecLinkId)}">Reject</button>` : ''}</div></li>`).join('')}</ol>`;
