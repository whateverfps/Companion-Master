...
// Discipline to specification sections mapping
const disciplineSpecs = {
  ...
  'Fire Protection': ['10 11 00', '10 12 00', '10 13 00', '10 14 00', '10 21 00', '10 22 00', '10 23 00', '10 24 00', '10 25 00', '10 26 00', '10 27 00', '10 28 00'],
...
catalog.sheets.forEach((sheet, index) => {
  const specSections = disciplineSpecs[sheet.discipline] || [];
  const links = specSections.map((sectionNumber, linkIndex) => ({
    linkId: `link-${index + 1}-${linkIndex + 1}`,
    ...
    sectionNumber,
    sectionTitle: specTitles[sectionNumber] || sectionNumber,
    origin: 'discipline-mapping',
    status: 'suggested',
    confidence: 0.7,
    evidenceSource: 'Discipline-based specification mapping',
    evidenceText: `Bedford specification ${sectionNumber} — ${specTitles[sectionNumber] || sectionNumber}`,
    reason: `Discipline-based suggestion (${sheet.discipline})`
  }));
...
console.log('Generated complete Building 61 specification links mapping:');
